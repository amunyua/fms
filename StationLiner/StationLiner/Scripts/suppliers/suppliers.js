...
//    alert("ok");
    ...
    $('#installationForm,#installationForm1')
        .formValidation({
            framework: 'bootstrap',
            icon: {
...
//       
        ...
        .bootstrapWizard({
            ...
            onNext: function (tab, navigation, index) {
                ...
                var formId = navigation.context.attributes["0"].value;
//                console.log(formId);
                var numTabs = $('#'+formId).find('.tab-pane').length,
                    isValidTab = validateTab(index - 1,formId);
                if (!isValidTab) {
                    return false;
                }
//                alert("next");
                $('#' + formId).find('.form-wizard').children('li').eq(index - 1).addClass(
                    'complete');
                $('#' + formId).find('.form-wizard').children('li').eq(index - 1).find('.step')
...
//                    alert("last");
            ...
            onPrevious: function (tab, navigation, index) {
                ...
                var fv = $('#'+formId).data('formValidation'), // FormValidation instance
                    // The current tab
                    $tab = $('#' + formId).find('.tab-pane').eq(index + 1);
...
//                alert('tab');
    ...
    function validateTab(index, formValue) {
        var fv = $('#'+formValue).data('formValidation'), // FormValidation instance
            // The current tab
            $tab = $('#' + formValue).find('.tab-pane').eq(index);
//        console.log($tab);
