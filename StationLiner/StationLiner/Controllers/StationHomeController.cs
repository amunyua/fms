...
//                db.InventoryPurchase.FirstOrDefault(x => x.StationId == Library.ChannelId && x.IsCompleted == false && x.IsCanceled == false);
        ...
        /// <returns></returns>
        public JsonResult UpdateFuelLevels(List<TankReadingModels> trModels)
        {
            ...
            try
            {
                if (trModels.Any())
                {
                    var shiftId = db.Shifts.FirstOrDefault(x => x.IsActive && x.StationId == Library.ChannelId);
                    foreach (var reading in trModels)
                    {
                        TankReading tankReading = new TankReading
                        {
                            ...
                            DateCreated = DateTime.Now,
                            ActualDateCreated = DateTime.Now,
                            ShiftId = (shiftId != null)? shiftId.Id : default(long?) ,

                            ReferenceDescription = "Dip Reading",
                        };
                        db.TankReadings.Add(tankReading);
                    }

                    db.SaveChanges();
                    status = "success";
                    message = "Fuel levels have been updated";
                }
            }
            catch (Exception e)
            {
            ...
            return Json(new{ status = status, message = ex});
        }
        /// <summary>
        ...
        /// 
       [HttpGet]
        public ActionResult FuelLevels()
        {
            
            return View(db.TankReadings.Where(x=>x.Tank.StationId == Library.ChannelId).Include(x=>x.Tank).OrderByDescending(x=>x.Id).Take(10).ToList());
        }
    }
}