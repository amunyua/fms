...
namespace StationLinerMVC.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        //function to allocate or remove channel from a user
        [HttpPost]
        public JsonResult AllocateDeChannel()
        {
            var action = Request.Form["action"];
            var userId = int.Parse(Request.Form["UserId"]);
            var channelId = int.Parse(Request.Form["ChannelId"]);
            if (action == "assign")
            {
                if (!db.UserAllocatedChannels.Any(x => x.StationId == channelId && x.UserId == userId))
                {
                    var channel = db.UserAllocatedChannels.Add(
                        new UserAllocatedChannels() {StationId = channelId, UserId = userId});
                    db.SaveChanges();
                }
            ...
            {
                var record = db.UserAllocatedChannels.Where(x => x.StationId == channelId && x.UserId == userId);
                if (record.Any())
                {
                    db.UserAllocatedChannels.Remove(record.First());
                    db.SaveChanges();
        ...
        [HttpPost]
        public JsonResult CheckAllocation()
        {
            var success = "unallocated";
            var userId = int.Parse(Request.Form["UserId"]);
            var channelId = int.Parse(Request.Form["ChannelId"]);
            if (db.UserAllocatedChannels.Any(x => x.UserId == userId && x.StationId == channelId))
            {
                success = "allocated";
            }
            return Json(success);
       ...
       /* public ActionResult Register()
        ...
        public async Task<ActionResult> ChangePassword(ChangePasswordViewModel model)
        {
            ...
            var user = db.Users.Find(Library.UserId);
            if (user == null)
            {
                return View(model);
            ...
            //encrypt password
            String passwordEncrypt = EncryptDecrypt.Encrypt(model.OldPassword, "@#Df4190^");
            if (user.PasswordHash != passwordEncrypt)
            {
                ViewBag.Message = "Your old password does not match the one in the records";
                return View(model);
            }
            user.PasswordHash = EncryptDecrypt.Encrypt(model.NewPassword, "@#Df4190^");
            try
            {
                db.SaveChanges();
...
//            ViewBag.LockoutMessage = "Your password has been changed successifully";
//            ViewBag.Link = HtmlHelper.GenerateLink(this.ControllerContext.RequestContext, System.Web.Routing.RouteTable.Routes, "Click here to login", null, "Login", "Account", null, null);
